...
class VacationRequestType extends AbstractType
{
    ...
    {
        $todayPlusOneDay = (new \DateTime())->modify('+1 day');
        if ($date < $todayPlusOneDay) {
            $context->buildViolation('Ending date cannot be before before atoday\'s date.')
                ->atPath('endDate')
                ->addViolation();
        }
    }

    public function checkEndingDate($date, ExecutionContextInterface $context)
    {
        $todayPlusOneDay = (new \DateTime())->modify('+1 day');
        if ($date < $todayPlusOneDay) {
            $context->buildViolation('Starting date cannot be before today\'s date.')
                ->atPath('startDate')
                ->addViolation();
        }
    }
