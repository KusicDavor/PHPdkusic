...
class TeamType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $approverRoles = $options['approverRoles']
        ...
        $builder
            ...
            ->add('teamLeader', EntityType::class, [
                ...
                'query_builder' => function (EntityRepository $er) use ($team) {
                    return $er->createQueryBuilder('u')
                        ->leftJoin('u.role', 'r')
                        ->andWhere('r.roleTag = :roleTag')
                        ->andWhere('u.team = :teamId')
                        ->setParameter('roleTag', 'ROLE_TEAM_LEADER')
            ...
            ->add('projectLeader', EntityType::class, [
                ...
                'placeholder' => 'Select project leader',
                'required' => false,
                'query_builder' => function (EntityRepository $er) use ($team, $approverRoles) {
                    return $er->createQueryBuilder('u')
                        ->leftJoin('u.role', 'r')
                        ->andWhere('r.roleTag = :roles')
                        ->andWhere('u.team = :teamId')
                        ->setParameter('roles', $approverRoles)
                        ->setParameter('teamId', $team->getId());
                },
            ]);
