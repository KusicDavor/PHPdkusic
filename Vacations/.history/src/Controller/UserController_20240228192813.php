...
class UserController extends AbstractController
{
    ...
    public function edit(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordEncoder): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $user = $form->getData();
            $this->checkOnTeamChange($form, $user, $entityManager);
            $entityManager->flush();
            return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
        }
    ...
    private function checkOnTeamChange(Form $form, User $updatedUser, EntityManagerInterface $entityManager): void
    {
        $newPassword = $form->get('mail')->getData();
        $oldUser = $entityManager->getRepository(User::class)->find($updatedUser->getId());

        $oldMail = $oldUser->getEmail();
        $newMail = $updatedUser->getEmail();

        error_log($oldMail . "------------------------" . $newMail);
