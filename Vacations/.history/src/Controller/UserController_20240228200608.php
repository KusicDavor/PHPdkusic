...
class UserController extends AbstractController
{
    ...
    public function edit(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordEncoder): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->checkOnTeamChange($user, $oldUser, $entityManager);
            $entityManager->flush();
            return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
        }
    ...
    private function checkOnTeamChange(User $newUser, User $oldUser, EntityManagerInterface $entityManager): void
    {
        
        error_log($newUser->getName() . "------------------------");
        error_log($oldUser->getName() . "------------------------"); */
