...
class UserController extends AbstractController
{
    ...
    {
        $oldTeamName = $user->getTeam()->getTeamName();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $team = $form->get('user')->getData();

            error_log($team->getTeamName() . "------------------------");
            error_log($oldTeamName . "------------------------");

            $this->checkOnTeamChange($oldTeamName, $user, $entityManager);
            $entityManager->flush();
            return $this->redirectToRoute('app_user_index', [], Response::HTTP_SEE_OTHER);
        }
    ...
    private function checkOnTeamChange(string $name, User $newUser, EntityManagerInterface $entityManager): void
    {
        /*         $user = $this->getUser();

        error_log($name . "------------------------");
        error_log($newUser->getName() . "------------------------"); */
