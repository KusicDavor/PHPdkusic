...
class VacationRequestController extends AbstractController
{
    ...
    public function delete(Request $request, VacationRequest $vacationRequest, EntityManagerInterface $entityManager): Response
    {
        ...
        return $this->redirectToRoute('app_vacation_request_edit', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/approve', name: 'app_vacation_approve', methods: ['POST'])]
    public function approve(Request $request, VacationRequest $vacationRequest, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid('approve' . $vacationRequest->getId(), $request->request->get('_token'))) {
            $vc = $vacationRequest;
            error_log('----------------------------------------------' . $vacationRequest->getId() . '--------------------------------------');
        }
        return $this->redirectToRoute('app_vacation_request_index', [], Response::HTTP_SEE_OTHER);
    }

    #[Route('/decline', name: 'app_vacation_decline', methods: ['POST'])]
    public function decline(Request $request, VacationRequest $vacationRequest, EntityManagerInterface $entityManager): Response
    {
        return $this->redirectToRoute('app_vacation_request_index', [], Response::HTTP_SEE_OTHER);
    }
}
