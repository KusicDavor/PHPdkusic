...
class UserController extends AbstractController
{
    ...
    public function index(UserRepository $userRepository): Response
    {
        ...
        return $this->render('user/index.html.twig', [
            'users' => $userRepository->findAll(), 'origin' => $origin
        ]);
    }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($oldTeam !== $user->getTeam()) {
                if ($oldTeam) {
                    ...
                    // $this->removeUserFromTeam($oldTeam, $user, $entityManager);
                }
                if ($user->getTeam()) {
                    // $this->addUserToTeam($user, $entityManager);
                }
            }
    ...
    // private function addUserToTeam(User $user, EntityManagerInterface $entityManager): void
    // {
    //     // dodaj usera u novi tim (jer ne povlači automatski iz nekog razloga)
    //     $teamRepository = $entityManager->getRepository(Team::class);
    //     $team = $teamRepository->find($user->getTeam()->getId());
    //     $team->addUser($user);
