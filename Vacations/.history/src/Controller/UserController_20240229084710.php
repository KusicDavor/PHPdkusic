...
class UserController extends AbstractController
{
    ...
    public function edit(Request $request, User $user, EntityManagerInterface $entityManager, UserPasswordHasherInterface $passwordEncoder): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $user = $form->getData();
            $teamToUpdate = $this->checkOnTeamChange($oldTeam, $user->getTeam(), $entityManager);
            error_log($teamToUpdate->getTeamName());
    ...
    private function checkOnTeamChange(?Team $oldTeam, ?Team $newTeam, EntityManagerInterface $entityManager): Team
    {
        if ($oldTeam !== $newTeam) {
