...
use App\Events\VacationRequestApprovedListener as EventsVacationRequestApprovedListener;
...
class VacationRequestController extends AbstractController
{
    ...
    public function index(VacationRequestRepository $vacationRequestRepository): Response
    {
        return $this->render('vacation_request/index.html.twig', [
            'vacation_requests' => $vacationRequestRepository->findBy([], ['startDate' => 'ASC'])
        ]);
    }
    ...
    public function approve(Request $request, VacationRequest $vacationRequest, EntityManagerInterface $entityManager, EventDispatcherInterface $dispatcher): Response
    {
    ...
    #[Route('/decide/{id}', name: 'app_vacation_decide', methods: ['POST'])]
    public function subtractVacationDays(Request $request, VacationRequest $vacationRequest, EntityManagerInterface $entityManager, EventDispatcherInterface $dispatcher): Response
    {

        if ($vacationRequest->isTeamLeadAction() || $vacationRequest->isProjectLeadAction()) {
            $event = new EventsVacationRequestApprovedListener();
