...
<?php
	...
	if(isset($_GET['slikap'])){
		$id=$_GET['slikap'];
		$sql="SELECT slika.naziv,url,slika.opis,datum_vrijeme_slikanja,ime,slika.korisnik_id,prezime,planina.planina_id,planina.naziv FROM slika,korisnik,planina
				WHERE slika.korisnik_id = korisnik.korisnik_id AND slika.planina_id=planina.planina_id AND slika.naziv='$id'";
	...
	if(isset($_POST['reset']))header("Location:slikap.php");
?>

<form method="POST" action="<?php if(isset($_GET['slika']))echo "slikap.php?korisnik=$id";else echo "slikap.php";?>">
	<table>
		<tbody>
			<tr>
				<td colspan="2">
			...
			<tr>
				...
				<td>
					<input readonly=readonly type="text" name="datum_vrijeme_slikanja" id="datum_vrijeme_slikanja" value="<?php if(!isset($_POST['datum_vrijeme_slikanja']))echo $datum_vrijeme_slikanja=date("d.m.Y H:i:s",strtotime($datum_vrijeme_slikanja)); else echo $_POST['datum_vrijeme_slikanja'];?>"
