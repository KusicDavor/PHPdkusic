    ...
    foreach (Router::$routes as $route) {
      if ($this->matchRoute($url, $route) && $route->method === $method) {
        ...
        if(is_callable($route->callback)) {
          ...
          Router::$response = new Response(405, "Metoda ne postoji.");
        }
      }        
    }
    Router::$response = new Response(404, "Stranica ne postoji.");
    Router::$response->send();
    return Router::$response;
  }
  ...
  public function prikaziOsobu() {
    ...
    if (preg_match($pattern, $url, $matches)) {
        ...
        if (Router::$request->getParameter("ime") == $parametarIme) {
          Router::$response->content = Router::dohvatiParametreReq();
        } else {
          Router::$response->statusCode = 404;
          Router::$response->content = "Osoba ne postoji.";
        }
      }
    Router::$response = new Response(200, $content);
  }
}