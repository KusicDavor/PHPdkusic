...
class Router {
  ...
  public static function handleRequest(Request $request) : ResponseInterface {
    ...
    $method = Router::$request->getParameter('method');
    $url = Router::$request->getParameter('url');
    $postoji = 0;
  ...
  public static function dohvatiParametreReq(Request $request) : string {
    ...
    public static function default() {
      if (Router::$request->getParameter("method") == "GET") {
        Router::$request = Router::srediGETzahtjev(Router::$request);
      }
  ...
  public static function prikaziOsobu() {
    ...
    if (preg_match($pattern, $url, $matches)) {
        ...
        if (Router::$request->getParameter("method") == "GET") {
          Router::$request = new Request(Router::srediGETzahtjev(Router::$request));
        }
