...
class Router {
  ...
  public static function obradiPOST() : ResponseInterface {
    $ime = Router::$request->getParam('ime');
    $spol = Router::$request->getParam('spol');
    $dob = Router::$request->getParam('dob');
    $content = "Osoba: $ime<br>Spol: $spol<br>Dob: $dob<br>Osoba uspješno obrađena.";
    $response = new Response(300, Router::$request->getParameter("ime"));
    $response->send();
    return $response;
  }
