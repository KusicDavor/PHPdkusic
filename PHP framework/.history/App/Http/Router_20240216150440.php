...
class Router {
  ...
  public function matchRoute(string $url, Route $route): bool {
    $pattern = Router::buildPatternFromUrlbuildPatternFromUrl($route->url);
  ...
  public static function prikaziOsobu() : ResponseInterface {
    ...
    $pattern = $this->buildPatternFromUrl(Router::$route->url);
    if (preg_match($pattern, $url, $matches)) {
        $variablePart = $matches[1]; // Extracted variable part
        echo "Variable part: $variablePart";
    } else {
        echo "No match found";
    }
