...
class Router {
  ...
  public static function createWithRoutes(array $routes) {
    $router = new self(); // Instantiate Router object
    $router->routes = $routes; // Set routes
    return $router;
}
  public function acceptRequest(Routes $request) {
    $this->request = $request;
    $this->handleRequest();
  }
