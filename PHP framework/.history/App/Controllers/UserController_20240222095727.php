...
class UserController {
    ...
    public static function index(Request $request) : ResponseInterface {
        ...
        // osigurava da se nakon brisanja prikaže /, a ne IndexController metoda
        ...
        $ = self::returnAllResults($limit);
        $htmlContent = $twig->render('.html.twig', ['' => $]);
    ...
    public static function returnSingleResult(int $id) {
        $connection = Connection::getInstance();
        $connection->connect();
        $query = "SELECT * FROM  WHERE id = :value";
        $params = [':value' => $id];
     ...
     public static function returnAllResults(?int $limit = null) {
        ...
        $ = $connection->fetchAssocAll($query, $limit);
        return $;
    }
}