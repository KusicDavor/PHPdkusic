...
class KorisnikController {
    ...
    public static function index(Request $request) : ResponseInterface {
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10; // Postavite 10 kao zadani limit ako nije unesen
        var_dump($request);
        $loader = new FilesystemLoader('Resources/views/');
        $twig = new Environment($loader);
        $response = new Response(200, "");
        $request = Router::dohvatiParametreReq($request);
        $korisnici = self::vratiSveRezultate(3);
