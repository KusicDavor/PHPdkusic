...
class AuthController extends AbstractController
{
    ...
    public function __construct(MessageBusInterface $bus, EntityManagerInterface $entityManger, UserPasswordHasherInterface $passwordEncoder, JWTTokenManagerInterface $jwtManager, FilesystemAdapter $cache, TokenStorageInterface  $tokenStorage, MailerInterface $mailer)
    {
    ...
    public function exportUsersCsv(Request $request): Response
    {
        ...
        $user = $this->getUser();
        $exportAttempts = $cache->getItem('export_attempts_' . $user->getUserIdentifier())->get() ?? 0;
        ...
        $exportAttempts++;
        $cachedItem = $cache->getItem('export_attempts_' . $user)->set($exportAttempts)->expiresAfter(self::CACHE_TTL);
        $cache->save();
