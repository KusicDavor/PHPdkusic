...
class AuthController extends AbstractController
{
    ...
    public function __construct(EntityManagerInterface $entityManger, UserPasswordHasherInterface $passwordEncoder, JWTTokenManagerInterface $jwtManager, JwtTokenManager $jwtToken, FilesystemAdapter $cache)
    {
    ...
    {
        if ($this->cache->getItem('cacheItem')) {
            $1 = $this->cache->getItem('cacheToken');
            $c2s = $this->cache->getItem('cacheToken');
            $this->cache->deleteItem('cacheToken');
            $cc = $this->cache->getItem('cacheToken');
