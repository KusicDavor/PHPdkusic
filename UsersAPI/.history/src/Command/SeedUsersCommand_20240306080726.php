...
class SeedUsersCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->passwordEncoder->hashPassword($faker, $this->entityManager->find(User::class), $this->entityManager->find(User::class));

        for ($i = 0; $i < 100; $i++) {
            $user = new User();
            $user->setName($faker->name);
            $user->setPassword($this->passwordEncoder->getPasswordHasher()->hash($user, $faker->password));
